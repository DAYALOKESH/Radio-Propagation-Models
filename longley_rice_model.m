...
%   Date: December 2024
    ...
    addParameter(p, 'maxDistance', 3840, @(x) isnumeric(x) && x > 0);         % meters
    ...
    for i = 1:num_points
        % Start with free space path loss
        base_loss_dB = freespace_loss(i);
        
        % Apply reflection modification (can be positive or negative)
        % reflection_loss already contains the effect relative to free space
        total_loss_with_reflection = base_loss_dB + reflection_loss(i);
        
        % Apply diffraction loss (always additional loss)
        total_pathloss(i) = total_loss_with_reflection + diffraction_loss(i);
        ...
        total_pathloss(i) = min(total_pathloss(i), freespace_loss(i) + 60);   % Max 60 dB additional loss
    end
...
function reflection_loss = calculate_ground_reflection(distances, terrain_heights, tx_height, rx_height, lambda, conductivity, permittivity, polarization)
    ...
    for i = 1:num_points
        ...
        % Reflection effect: difference from direct path alone (in dB)
        % Negative values mean reflection enhances signal, positive means reduction
        if E_total_mag > 0 && E_direct > 0
            reflection_loss(i) = 20 * log10(E_direct / E_total_mag);
        else
            reflection_loss(i) = 40;  % Large loss if field cancels out
        end
...
function diffraction_loss = calculate_diffraction_loss(distances, terrain_heights, tx_position, rx_height, lambda, refractivity)
    ...
    for i = 1:num_points
        ...
        edges = find_knife_edges(path_distances, path_heights, tx_x, tx_y, rx_x, rx_y, lambda);
...
function edges = find_knife_edges(path_distances, path_heights, tx_x, tx_y, rx_x, rx_y, lambda)
    ...
    for i = 2:length(clearances)-1
        ...
        if d1 > 0 && d2 > 0 && total_distance > 0
            ...
            if clearances(i) < -0.2 * F1_radius  % 20% of first Fresnel zone
